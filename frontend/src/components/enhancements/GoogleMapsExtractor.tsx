      ...
      const poll = setInterval(async () => {
        try {
          ...
          if (s.status === 'completed' || s.status === 'failed') {
            ...
            if (s.status === 'failed') {
              setErrorMessage((s as any).error_message || 'Extraction failed. Make sure you have a Serper API key configured in Settings, or Google Chrome installed.');
            }
            setLoading(false);
          }
        } catch {
      ...
      <div className="max-w-[720px] mx-auto space-y-8">
        ...
        {status === 'failed' && !loading && (
          <div className="rounded-xl bg-red-500/5 border border-red-500/20 p-4">
            <p className="text-sm text-red-400">{errorMessage || 'Extraction failed. Make sure you have a Serper API key configured in Settings, or Google Chrome installed.'}</p>
          </div>
        )}
