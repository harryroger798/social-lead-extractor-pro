...
import { BookOpen, Search, Loader2, MapPin } from 'lucide-react';
import { searchDirectories, getExtractionStatus } from '@/lib/api';

const SOURCES = [
  { id: 'yellowpages', name: 'YellowPages', color: 'text-yellow-400' },
  { id: 'yelp', name: 'Yelp', color: 'text-red-400' },
];

export default function DirectoryScraper() {
  const [query, setQuery] = useState('');
  const [location, setLocation] = useState('');
  const [selectedSources, setSelectedSources] = useState<string[]>(['yellowpages', 'yelp']);
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    setStatus('Starting directory search...');
    setResult(null);

    try {
      const res = await searchDirectories({
        query: query.trim(),
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold text-text-primary">Directory Scraper</h2>
          <p className="text-sm text-text-muted">Search YellowPages & Yelp for business leads</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-bg-card rounded-xl border border-border p-5 space-y-5">
          <div>
            <label className="block text-xs font-medium text-text-muted mb-2">Business Type / Query</label>
            <input
              type="text"
              value={query}
          ...
          <div>
            <label className="block text-xs font-medium text-text-muted mb-2">Sources</label>
          ...
          >
            {loading ? (
              ...
              <><Search className="w-4 h-4" /> Search Directories</>
            )}
          </button>

          {status && (
            <p className="text-xs text-text-muted text-center">{status}</p>
          )}
        </div>
          ...
          {result ? (
            <div className="space-y-3">
              <div className="p-3 rounded-lg bg-accent/5 border border-accent/10">
                ...
                <p className="text-xs text-text-muted">Total Leads Found</p>
              </div>
            ...
            <div className="text-center py-8 text-text-muted">
              ...
              <p className="text-sm">Search directories to find business leads</p>
