...
class WritingToolsService:
    ...
    def _load_t5_model(self):
        """Load T5 model (ONNX quantized if available, PyTorch fallback)."""
        ...
        try:
            ...
            if os.path.exists(onnx_dir) and any(f.endswith('.onnx') for f in os.listdir(onnx_dir)):
                self._t5_tokenizer = AutoTokenizer.from_pretrained(onnx_dir)
                self._t5_model = ORTModelForSeq2SeqLM.from_pretrained(onnx_dir)
                logger.info("T5 ONNX quantized model loaded for writing tools")
                return
            ...
            self._t5_model.eval()
            logger.info("T5 model loaded for writing tools (PyTorch fallback)")
        except Exception as e:
            logger.error(f"Failed to load T5 model: {e}")
    
    def _load_general_t5_model(self):
        """Load general-purpose Flan-T5 model (ONNX quantized for speed)."""
        ...
        try:
            onnx_dir = os.path.join(settings.MODELS_DIR, 'flan-t5-base-onnx')
            if os.path.exists(onnx_dir) and any(f.endswith('.onnx') for f in os.listdir(onnx_dir)):
                self._general_t5_tokenizer = AutoTokenizer.from_pretrained(onnx_dir)
                self._general_t5_model = ORTModelForSeq2SeqLM.from_pretrained(onnx_dir)
                logger.info("Flan-T5 ONNX quantized model loaded successfully")
                return True
            ...
            self._general_t5_model.eval()
            logger.info("Flan-T5 model loaded from local storage (PyTorch fallback)")
            return True
        except Exception as e:
    ...
    def _load_grammar_model(self):
        """Load CoEdIT-large grammar model (ONNX quantized for speed)."""
        ...
        try:
            ...
            if os.path.exists(onnx_dir) and any(f.endswith('.onnx') for f in os.listdir(onnx_dir)):
                ...
                logger.info("CoEdIT-large ONNX quantized model loaded successfully")
                return True
            ...
            self._grammar_model.eval()
            logger.info("CoEdIT-large model loaded from local storage (PyTorch fallback)")
            return True
        except Exception as e:
    ...
    def _generate_with_t5(self, prompt: str, max_length: int = 256, min_length: int = 10) -> Optional[str]:
        """Generate text using HF API (primary) → ONNX local (fast) → SageMaker (fallback)."""
        try:
        ...
        try:
            if self._load_general_t5_model() and self._general_t5_model is not None and self._general_t5_tokenizer is not None:
                ...
                if generated_text and len(generated_text.strip()) >= 5 and generated_text.strip().lower() != prompt.strip().lower():
                    logger.info("Flan-T5 generation via ONNX local successful")
                    return generated_text.strip()
        except Exception as e:
            logger.warning(f"ONNX local flan-t5 failed: {e}")
        ...
        try:
            ...
            logger.warning(f"SageMaker flan-t5 also failed: {e}")
